...
module.exports = {
    ...
    */
     ...
     * @param {requestCallback} [callback] Optional callback method to be called instead of a promise.
     * @example getBusPredictions(1001195).then(data => predictions = data.Predictions, stopName = data.StopName)
     * @returns {Promise|undefined} A promise that if successful, returns an object containing
     *  a "Predictions" property, and a "Stop Name" property, which maps to a string of the bus
     *  stop's full name (eg. "7th-and-Massachussets"). If rejected, contains as error object. Not returned if 
     *  a callback is given.
    */
    ...
    /**
     * Get bus route incident alerts.
     * @param(string) [route] Bus route to get incident alerts for. If omitted, returns incident alerts for all bus routes. 
     * @param {requestCallback} [callback] Optional callback method to be called instead of a promise.
     * @example getBusIncidents('D2').then(data => incidents = data.BusIncidents)
     * @returns {Promise|undefined} A promise that if successful, returns an object containing
     *  bus arrival time prediction information. If rejected, contains as error object. Not returned if 
     *  a callback is given.
    */

    getBusIncidents: function(route, callback){
        var url = endpoints.bus.incidents;
