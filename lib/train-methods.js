...
module.exports = {
    ...
    /**
     * Get train incident alerts.
     * @param {requestCallback} [callback] Optional callback method to be called instead of a promise.
     * @example getTrainIncidents().then(data => incidents = data.Incidents)
     * @returns {Promise|undefined} A promise that if successful, returns an object with an 'Incidents' property mapping to an array
     *      of train prediction information objects. If rejected, contains an error object. Not returned if callback if given.
     */

    getTrainIncidents: function(callback){
        var url = endpoints.train.incidents;
        var promise = this._makeApiRequest(url);
